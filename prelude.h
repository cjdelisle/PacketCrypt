...
#define OUT(x) do { DEBUGF("out1(%08x) %d\n", (x), hashctr); hashOut[hashctr] += (x); hashctr = (hashctr + 1) % HASH_SZ; } while (0)
...
#define IN(x) hashIn[(x) & (MEMORY_SZ - 1)]
#define MEMORY(loopVar, base, step, carry) memory[(base + ((loopVar + carry) * step)) & (MEMORY_SZ - 1)]
